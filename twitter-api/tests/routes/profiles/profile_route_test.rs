...
async fn test_route_create_profile_with_avatar() {
    ...
    let update_avatar_req = test::TestRequest
        ...
        .set_payload(payload)
        .to_request();
    let user_id = test::call_and_read_body_json::<_, _, i64>(&app, update_avatar_req).await;    

    let get_profile_req = test::TestRequest
        ::get()
        .uri(&format!("/v1/profile/{}", user_id))
        .to_request();
    let get_profile = test
        ::call_and_read_body_json::<_, _, Option<ProfileResponder>>(&app, get_profile_req).await
        .unwrap();

    assert!(get_profile.avatar.unwrap() == avatar);
}
